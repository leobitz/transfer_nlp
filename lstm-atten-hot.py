...
class Encoder(tf.keras.Model):
    ...
    def call(self, w, f, hidden):
        output, state_h, state_c = self.gru(w, initial_state=hidden)
        #state = [state_h, state_c]
        feat = self.fc1(f)
        state_h = tf.concat([state_h, feat], axis=1)
        state_h = self.fc2(state_h)
        state_c = tf.concat([state_c, feat], axis=1)
        state_c = self.fc2(state_c)
        return output, [state_h, state_c], feat

    def initialize_hidden_state(self):
        return [tf.zeros((self.batch_size, self.enc_units), dtype=tf.float32), tf.zeros((self.batch_size, self.enc_units), dtype=tf.float32)]
...
class BahdanauAttention(tf.keras.Model):
    ...
    def call(self, query, values):
        ...
        # we get 1 at the last axis because we are applying score to self.V
        # the shape of the tensor before applying self.V is (batch_size, max_length, units)
        # print(query[0].shape, values[0].shape)
        score = self.V(tf.nn.tanh(
            self.W1(values) + self.W2(hidden_with_time_axis)))
...
def train_step(root, feature, dec_input, target, enc_hidden):
    ...
    with tf.GradientTape() as tape:
        ...
        for t in range(target.shape[1]):
            # print(dec_input[:, t].shape, dec_hidden[0].shape,enc_output.shape)
